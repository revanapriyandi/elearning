...
@section('content')
    <div class="row">
        <div class="col-lg-6">
            <h4 class="text-white">{{ __('Edit Data Ujian') }}</h4>
        </div>
    </div>
    <form action="{{ route('banksoal.update', $soal->id) }}" method="POST" id="form">
        @csrf
        @method('PUT')
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-12 col-sm-6">
                                ...
                                <input class="form-control  @error('nama_ujian') is-invalid @enderror" type="text"
                                    value="{{ $soal->judul }}" name="nama_ujian" autofocus
                        ...
                        <div class="row">
                            <div class="col-6">
                                ...
                                <input class="form-control @error('waktu_mulai') is-invalid @enderror" type="datetime-local"
                                    value="{{ $soal->waktu_mulai }}" name="waktu_mulai">
                                @error('waktu_mulai')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                            ...
                            <div class="col-6">
                                ...
                                <input class="form-control @error('waktu_berakhir') is-invalid @enderror"
                                    type="datetime-local" value="{{ $soal->waktu_berakhir }}" name="waktu_berakhir">
                        ...
                        <div class="row">
                            <div class="col-12">
                                ...
                                <textarea class="form-control @error('deskripsi') is-invalid @enderror" name="deskripsi" rows="3">{{ $soal->deskripsi }}</textarea>
                                @error('deskripsi')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                        ...
                        <div class="row">
                            <div class="col-12">
                                ...
                                <select name="kelas[]" class="form-control @error('kelas') is-invalid @enderror"
                                    multiple="multiple">
                                    <option value="all" {{ $soal->kelas_id == 'all' ? 'selected' : '' }}>
                                        {{ __('Semua Kelas') }}</option>
                                    @php
                                        $data = $soal->kelasTugasQuiz->pluck('kelas_id')->toArray();
                                    @endphp
                                    @foreach ($kelas as $item)
                                        <option value="{{ $item->id }}"
                                            {{ in_array($item->id, $data) ? 'selected' : '' }}>
                            ...
                            <div class="col-12">
                                ...
                                <select name="mapel" class="form-control @error('mapel') is-invalid @enderror">
                                    @foreach ($mapel as $item)
                                        <option value="{{ $item->id }}"
                                            {{ $soal->mata_pelajaran_id == $item->id ? 'selected' : '' }}>
                                ...
                                <div class="col-6">
                                    ...
                                    <div class="form-check form-switch">
                                        <input class="form-check-input @error('is_aktif') is-invalid @enderror"
                                            ...
                                            {{ $soal->is_aktif ? 'checked' : '' }} required>
                                    </div>
                                    @error('is_aktif')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                ...
                                <div class="col-6">
                                    ...
                                    <div class="form-check form-switch">
                                        <input class="form-check-input @error('is_terbitkan_nilai') is-invalid @enderror"
                                            ...
                                            {{ $soal->is_terbitkan_nilai ? 'checked' : '' }} required>
                                    </div>
